...
sims <- read.csv("~/UBC/Mouse_BGS_RecombinationRateEvolution/Simulations/simple_simResults_N1000.output.csv")
...
ggplot(data = sims, aes( x= (gen-15000)/(2000), y = pi, group = R))+
...
B <- read.csv("~/UBC/Mouse_BGS_RecombinationRateEvolution/TheoreticalExpectation/B_fixed_s.csv")
sims_fixed <- read.csv("~/UBC/Mouse_BGS_RecombinationRateEvolution/Simulations/simple_simResults_N1000_fixed.csv")
names(sims_fixed) <- c("pi","pos","gen","rep","R")

B_fixed_plot <- ggplot(data = sims_fixed[sims_fixed$gen>26000,], aes( x = pos/1000, y = pi/0.01, col = as.factor(R)))+
  stat_summary(fun = "mean", aes(colour = as.factor(R)), size = 2, geom = "point")+
...
BGS_fixed_s_time_plot <- ggplot(data = sims_fixed, aes( x= (gen-15000)/(2000), y = pi, group = R))+
  facet_grid(.~pos_label)+
  geom_line(data= BGS, aes(x = gen, y = B*0.01, col = as.factor(R)), lty =2)+
  scale_y_continuous(expression(pi))+
  scale_x_continuous("Time since recombination rate changed (2N generations)")+
...
BGS_fixed_s_time_plot_singlePanel <- ggplot(data = sims_fixed[sims_fixed$pos_label == "10Kbp away",], aes( x= (gen-15000)/(2000), y = pi, group = R))+
...
ggplot(data = recRates, aes( x= (generation-15000)/(2000), y = r_rate*4000, col = as.factor(R)))+
...
### For presentation
B_fixed_plot_1 <- ggplot(data = sims_fixed[sims_fixed$gen>26000,], aes( x = pos/1000, y = pi, col = as.factor(R)))+
#  stat_summary(fun = "mean", aes(colour = as.factor(R)), size = 2, geom = "point")+
#  stat_summary(fun.data = mean_se, aes(colour = as.factor(R)), geom = "errorbar", width = 0)+
...
B_fixed_plot_2 <- ggplot(data = sims_fixed[sims_fixed$gen>26000,], aes( x = pos/1000, y = pi, col = as.factor(R)))+
  #  stat_summary(fun = "mean", aes(colour = as.factor(R)), size = 2, geom = "point")+
  #  stat_summary(fun.data = mean_se, aes(colour = as.factor(R)), geom = "errorbar", width = 0)+
