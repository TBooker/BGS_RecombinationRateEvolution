...
## Make VCF files for simulation replicates
#python ../../bin/parseTrees_toVCF.py --tree ../../simple_map_constant_s0.01/

### Make the lookup table
#pyrho make_table --samplesize 20 --mu 5e-7 -o constantSizeSlim.hdf -p 1000,1000,1000 -t 1,100000 --approx

## Calculate
#pyrho hyperparam -n 20 --mu 5e-7 --blockpenalty 50,100 \
#        --windowsize 25,50 --logfile . --tablefile constantSizeSlim.hdf \
#        --num_sims 3 \
#	-p 1000,1000,1000 \
#	-t 1,100000 \
#	--outfile constantSizeSlim.hyperParams.txt
...
for s in $(ls $1/*vcf)
	 do
	 echo $s
	 pyrho optimize --tablefile constantSizeSlim.hdf \
        --vcffile $s \
        --outfile $s.rmap \
        --blockpenalty 50 --windowsize 25 \
        --logfile .
...
exit 0
## Below is the example analysis from the PyRho Repository
...
pyrho make_table -n 20 -N 25 --mu 2.5e-6 --logfile . --outfile ACB_n_20_N_40_lookuptable.hdf \
...
pyrho hyperparam -n 20 --mu 2.5e-6 --blockpenalty 50,100 \
